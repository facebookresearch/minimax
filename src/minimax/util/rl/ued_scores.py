...
def compute_episodic_stats(
    ...
    def _compute_metrics(carry, step):
        ...
        max_metrics = (
            _dones * jnp.maximum(max_metrics, ep_metric) + (1 - _dones) * max_metrics
        )
    ...
    (n_episodes, sum_ep_metrics, max_metrics, partial_metrics, partial_steps), _ = (
        jax.lax.scan(
            _compute_metrics,
            (n_episodes, sum_ep_metrics, max_metrics, partial_metrics, partial_steps),
            (metrics, dones),
...
def _compute_ued_scores(score_name: UEDScore, batch: namedtuple, info=None):
    ...
    """
    if score_name in [
        UEDScore.RELATIVE_REGRET,
        UEDScore.MEAN_RELATIVE_REGRET,
        UEDScore.POPULATION_REGRET,
    ]:
...
def compute_ued_scores(
    ...
    if score_name == UEDScore.RELATIVE_REGRET:
        scores = jnp.clip(
            max_env_returns_per_agent[1] - mean_env_returns_per_agent[0], 0
        )

    elif score_name == UEDScore.MEAN_RELATIVE_REGRET:
        scores = jnp.clip(
            mean_env_returns_per_agent[1] - mean_env_returns_per_agent[0], 0
        )
...
def compute_max_mc(batch, info):
    _, max_env_returns_per_agent = compute_episodic_stats(
        batch.rewards, batch.dones, time_average=False
    )
