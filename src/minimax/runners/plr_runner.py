...
class PLRRunner(DRRunner):
    ...
    def reset(self, rng):
        ...
        if self.n_devices == 1:
            ...
            plr_buffer = jax.tree_map(
                lambda x: x.repeat(self.n_devices, 1), plr_buffer
            )  # replicate plr buffer
            runner_state += (
                plr_buffer,
    ...
    def _efficient_grad_update(self, rng, train_state, train_batch, is_replay):
        ...
        if self.n_students == 1:
            ...
            self.n_students > 1
        ):  # Have to vmap all students + take only students that need updates
            _, dummy_stats = jax.vmap(
                lambda *_: self.student_pop.agent.get_empty_update_stats()
            )(np.arange(self.n_students))
            _train_state, stats = self.student.update(rng, train_state, train_batch)
            train_state, stats = jax.vmap(
                lambda cond, x, y: jax.tree_map(
                    lambda _cond, _x, _y: jax.lax.select(_cond, _x, _y), cond, x, y
                )
    ...
    def _compile_stats(self, update_stats, ep_stats, env_metrics=None, plr_stats=None):
        ...
        if plr_stats is not None:
            plr_stats = jax.vmap(lambda info: jax.tree_map(lambda x: x.mean(), info))(
                plr_stats
            )
    ...
    def run(
        ...
        rng, subrng = jax.random.split(rng)
        levels, level_idxs, is_replay, next_plr_buffer = self.plr_mgr.sample(
            subrng, train_state.plr_buffer, new_levels, n_level_samples
        )
        train_state = train_state.replace(plr_buffer=next_plr_buffer)
            ...
            if self.use_mutations:
                ...
                _all_levels = jax.vmap(
                    lambda x, y: _tree_util.pytree_merge(
                        x,
                        y,
                        start_idx=mutated_levels_start_idx,
                        src_len=n_level_samples,
                    ),
        ...
        result = self._eval_and_update_plr(
            ...
            level_idxs,
            train_state,
            update_plr=jnp.array([True] * self.n_students),
            parent_idxs=parent_idxs,
            dupe_mask=dupe_mask,
        )
        (
