...
def apply_ssm(
    Lambda_bar, B_bar, C_tilde, hidden, input_sequence, resets, conj_sym, bidirectional
):
...
class SequenceLayer(nn.Module):
    ...
    d_model: int
    activation: str = "gelu"
    layernorm_pos: str = None  # ['pre', 'post', None]
    step_rescale: float = 1.0
    ...
    def __call__(self, hidden, x, d):
        ...
        Args:
                 x (float32): input sequence (L, B, d_model)
                 d (bool): reset signal (L,B)
        Returns:
                output sequence (float32): (L, B, d_model)
        """
        is_one_step = len(hidden.shape) == 2
        if is_one_step:  # Add time axis
            hidden = hidden[jnp.newaxis, :]

        skip = x
        if self.layernorm_pos == "pre":
            x = self.norm(x)
    ...
    def initialize_carry(batch_size, hidden_size):
        return jnp.zeros((batch_size, hidden_size), dtype=jnp.complex64)
...
class S5EncoderStack(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = [
            SequenceLayer(
                ...
                layernorm_pos=self.layernorm_pos,
            )
            for _ in range(self.n_layers)
        ]

    def __call__(self, hidden, x, reset):
        """
        Compute the BxLxH output of the stacked encoder given an Lxd_input
        input sequence.
        Args:
        ...
        for i, layer in enumerate(self.layers):
            new_h, x = layer(hidden[i], x, reset)
            new_hiddens.append(new_h)

        return new_hiddens, x

    @staticmethod
    def initialize_carry(rng, batch_dims, hidden_dim, n_layers):
        # Use a dummy key since the default state init fn is just zeros.
        return [
            jnp.zeros((*batch_dims, hidden_dim), dtype=jnp.complex64)
            for _ in range(n_layers)
        ]


BatchS5EncoderStack = nn.vmap(
    S5EncoderStack,
    in_axes=(1, 1, 1),
    out_axes=1,
...
def make_s5_encoder_stack(
    ...
    block_size = int(ssm_state_dim / n_blocks)

    Lambda, _, B, V, B_orig = make_DPLR_HiPPO(block_size)

    if conj_sym:
        block_size = block_size // 2
        ssm_state_dim = ssm_state_dim // 2
    ...
    return S5EncoderStack(
        ssm=ssm_init_fn,
        d_model=input_dim,
        n_layers=n_layers,
        activation=activation,
        layernorm_pos=layernorm_pos,
    )
