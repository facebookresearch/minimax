...
# ==== RL runner arguments.
train_runner_subparser = parser.add_subparser(name="train_runner")
train_runner_subparser.add_argument(
    "--n_students", type=int, default=1, help="Number of students in population."
)
train_runner_subparser.add_argument(
    "--n_parallel",
    type=int,
...
train_runner_subparser.add_argument(
    "--n_rollout_steps", type=int, default=250, help="Number of rollout steps."
)
train_runner_subparser.add_argument(
    "--lr", type=float, default=1e-4, help="Initial learning rate."
)
train_runner_subparser.add_argument(
    "--lr_final", type=float, default=None, nargs="?", help="Final learning rate."
)
train_runner_subparser.add_argument(
    "--lr_anneal_steps",
    type=int,
...
train_runner_subparser.add_argument(
    "--max_grad_norm", type=float, default=0.5, help="max norm of gradients."
)
train_runner_subparser.add_argument(
    "--adam_eps", type=float, default=1e-5, help="Adam eps."
)
train_runner_subparser.add_argument(
    "--track_env_metrics",
    type=str2bool,
...
train_runner_subparser.add_argument(
    "--discount", type=float, default=0.995, help="Student discount factor for rewards"
)
train_runner_subparser.add_argument(
    "--n_unroll_rollout",
    type=int,
...
train_runner_subparser.add_argument(
    "--render", type=str2bool, default=False, help="Whether to render."
)

# ------ AC-specific arguments -----
dr_subparser = parser.add_subparser(
    name="dr", prefix="dr", dependency={"train_runner": "dr"}, dest="train_runner"
)
...
plr_subparser = parser.add_subparser(
    name="plr", prefix="plr", dependency={"train_runner": "plr"}, dest="train_runner"
)
plr_subparser.add_argument(
    "--replay_prob", type=float, default=0.5, help="PLR replay probability."
)
plr_subparser.add_argument(
    "--buffer_size", type=int, default=128, help="PLR level buffer size."
)
plr_subparser.add_argument(
    "--staleness_coef", type=float, default=0.3, help="Staleness coefficient."
)
plr_subparser.add_argument(
    "--temp", type=float, default=1.0, help="Score distribution temperature."
)
plr_subparser.add_argument(
    "--use_score_ranks",
    type=str2bool,
...
plr_subparser.add_argument(
    "--use_robust_plr", type=str2bool, default=True, help="Use robust PLR."
)
plr_subparser.add_argument(
    "--use_parallel_eval",
    type=str2bool,
...
plr_subparser.add_argument(
    "--mutation_fn", type=str, default=None, help="Name of mutation function for ACCEL."
)
plr_subparser.add_argument(
    "--n_mutations",
    type=int,
...
student_rl_subparser = parser.add_subparser(name="student_rl", prefix="student")
student_rl_subparser.add_argument(
    "--entropy_coef", type=float, default=0.0, help="entropy term coefficient"
)
student_rl_subparser.add_argument(
    "--value_loss_coef",
    type=float,
...
student_ppo_subparser.add_argument(
    "--n_epochs", type=int, default=5, help="Number of PPO epochs."
)
student_ppo_subparser.add_argument(
    "--n_minibatches", type=int, default=1, help="Number of minibatches per PPO epoch."
)
student_ppo_subparser.add_argument(
    "--clip_eps", type=float, default=0.2, help="PPO clip parameter"
)
student_ppo_subparser.add_argument(
    "--clip_value_loss", type=str2bool, default=True, help="ppo clip value loss"
)
parser.add_dependent_argument(
    "--gae_lambda",
    type=float,
...
teacher_rl_subparser = parser.add_subparser(
    name="teacher_rl", prefix="teacher", dependency={"train_runner": ["paired"]}
)
...
student_model_parser = parser.add_subparser(name="student_model", prefix="student")
...
student_maze_model_parser.add_argument(
    "--hidden_dim", type=int, default=32, help="Student hidden dimension."
)
student_maze_model_parser.add_argument(
    "--n_hidden_layers",
    type=int,
...
student_maze_model_parser.add_argument(
    "--n_conv_filters", type=int, default=16, help="Number of CNN filters for student."
)
student_maze_model_parser.add_argument(
    "--n_scalar_embeddings",
    type=int,
...
student_maze_model_parser.add_argument(
    "--s5_n_blocks", type=int, default=1, help="Number of S5 blocks."
)
student_maze_model_parser.add_argument(
    "--s5_n_layers", type=int, default=4, help="Number of S5 encoder layers."
)
student_maze_model_parser.add_argument(
    "--s5_layernorm_pos", type=str, default=None, help="Layernorm pos in S5."
)
student_maze_model_parser.add_argument(
    "--s5_activation",
    type=str,
...
teacher_model_parser = parser.add_subparser(
    name="teacher_model", prefix="teacher", dependency={"train_runner": ["paired"]}
)
...
teacher_maze_model_parser.add_argument(
    "--hidden_dim", type=int, default=32, help="Teacher hidden dimension."
)
teacher_maze_model_parser.add_argument(
    "--n_hidden_layers",
    type=int,
...
teacher_maze_model_parser.add_argument(
    "--n_conv_filters", type=int, default=128, help="Number of CNN filters for teacher."
)
teacher_maze_model_parser.add_argument(
    "--scalar_embed_dim",
    type=int,
...
parser.add_argument(
    "--env_name", type=str, default="Maze", help="Environment to train on"
)
env_parser = parser.add_subparser(name="env")

# -------- UED environment arguments. --------
ued_env_parser = parser.add_subparser(name="ued_env")
...
env_maze_parser.add_argument(
    "--height", type=int, default=13, help="Height of training mazes."
)
env_maze_parser.add_argument(
    "--width", type=int, default=13, help="Width of training mazes."
)
env_maze_parser.add_argument(
    "--n_walls", type=int, default=25, help="Maximum number of walls in training mazes."
)
env_maze_parser.add_argument(
    "--replace_wall_pos",
    type=str2bool,
...
env_maze_all_parser = parser.add_subparser(
    name="maze_all", prefix="maze", dependency={"env_name": "Maze*"}, dest="env"
)
...
maze_ued_parser.add_argument(
    "--n_walls", type=int, default=25, help="Number walls the adversary can place."
)
maze_ued_parser.add_argument(
    "--set_agent_dir",
    type=str2bool,
...
parser.add_argument(
    "--xpid", default="latest", help="name for the run - prefix to log files"
)
parser.add_argument(
    "--log_dir", default="~/logs/minimax/", help="directory to save agent logs"
)
parser.add_argument(
    "--log_interval", type=int, default=1, help="log interval, one log per n updates"
)
parser.add_argument(
...
# Evaluation args.
eval_parser = parser.add_subparser(name="eval", prefix="test")
eval_parser.add_argument(
    "--n_episodes", type=int, default=10, help="Number of test episodes per environment"
)
eval_parser.add_argument(
    "--env_names", type=str, default=None, help="Test environments to evaluate on."
)
eval_parser.add_argument(
    "--agent_idxs",
    type=str,
...
# -------- wandb arguments. --------
wandb_parser = parser.add_subparser(name="wandb", prefix="wandb")
wandb_parser.add_argument(
    "--base_url", type=str, default="https://api.wandb.ai", help="wandb base url"
)
wandb_parser.add_argument("--api_key", type=str, default=None, help="wandb api key")
wandb_parser.add_argument("--entity", type=str, default="minqi", help="Team name")
wandb_parser.add_argument(
